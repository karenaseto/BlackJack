...
class PygameUI:
    ...
    def draw_cards(self, player, x, y):
        for i, card in enumerate(player.hand):
            image_filename = f"images/{str(card.rank.lower())}_{str(card.suit).lower()}_white.png"
    ...
    def update_display(self, game, reveal_dealer_card=False):
        ...
        self.draw_cards(game.player, 50, 300)
        ...
        while True:
            ...
            if action == 'hit':
                ...
                if self.player.calculate_hand_value() > 21:
                    ...
                    break
                # dealer will hit until it hits at least 17; no reason why, just a high enough number so the user won't always win
            elif action == 'stand':
                while self.dealer.calculate_hand_value() < 17:
                    self.dealer.add_card(self.deck.deal())

                self.display_game_status(reveal_dealer_card=True)
